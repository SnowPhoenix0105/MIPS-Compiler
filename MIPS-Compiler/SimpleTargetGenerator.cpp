...
void SimpleCodeGenerator::func_body()
{
	...
	for (size_t index = func_mid_index + 1; index < func_end_index; ++index)
	{
		...
		case IrHead::scanf:
			buffer << mips.sw("$v0", "$sp", stack_size - 8) << endl;
			buffer << mips.li("$v0", ir.elem[1] == IrType::_int ? mips.read_int : mips.read_cahr) << endl;
			buffer << mips.syscall() << endl;
			save_reg_to_var("$v0", ir.elem[0]);
			buffer << mips.lw("$v0", "$sp", stack_size - 8) << endl;
			break;
		case IrHead::printf:
		{
			bool str = ir.elem[0] != IrType::NIL;
			bool val = ir.elem[1] != IrType::NIL;
			buffer << mips.sw("$v0", "$sp", stack_size - 8) << endl;
			if (str)
			{
				const auto& label = string_label_table.at(ir.elem[0]);
			...
			if (str && val)
			{
				load_val_to_reg("$a0", ir.elem[1]);
				buffer << mips.li("$v0", ir.elem[2] == IrType::_int ? mips.print_int : mips.print_char);
				buffer << mips.syscall() << endl;
			}
			buffer << mips.li("$a0", '\n') << endl;
